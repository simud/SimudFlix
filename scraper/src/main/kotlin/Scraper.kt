...
object Scraper {
    ...
    fun setupHeaders(): Boolean {
        ...
        if (dataPage == null) {
            println("Errore: Impossibile trovare data-page. Verifica la struttura HTML di $MAIN_URL/archive")
            return false
        }

        val version = objectMapper.readValue<Map<String, Any>>(dataPage).get("version") as? String
        if (version == null) {
            println("Errore: Impossibile estrarre X-Inertia-Version da data-page")
            return false
        }
